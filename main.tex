...
Several model families are relevant for object detection in aerial and satellite imagery. General-purpose object detectors such as Faster R-CNN, YOLO, SSD, and RetinaNet form the basis for many remote-sensing approaches. Faster R-CNN represents the two-stage detection paradigm and is commonly used as a strong accuracy-oriented baseline. YOLO and SSD are one-stage detectors that are designed for faster inference by directly predicting object classes and bounding boxes from dense feature maps. RetinaNet also follows a one-stage design, but introduces Focal Loss to address the foreground-background imbalance that occurs during dense detection \cite{li2020object,lin2017focal}.

For aerial imagery, multi-scale feature extraction is particularly important. Feature Pyramid Networks (FPN) are therefore a key component in many modern detectors. FPNs build a feature hierarchy with semantically strong representations at multiple scales, allowing the detector to handle both small and large objects more effectively. This is especially relevant in remote sensing, where the same image can contain small vehicles, medium-sized ships, and large structures such as airports or harbors \cite{lin2017feature,ding2021object}.

A central model for oriented object detection is the RoI Transformer. It is based on a two-stage detection framework and addresses the mismatch between horizontal region proposals and arbitrarily oriented objects. Instead of directly relying on horizontal Regions of Interest (HRoIs), the RoI Transformer learns a transformation from HRoIs to rotated Regions of Interest (RRoIs). This transformation is performed by a trainable RRoI Learner. The resulting RRoIs are then used for rotation-aware feature extraction through Rotated Position-Sensitive RoI Align. This improves the alignment between region features and object geometry, especially in dense aerial scenes where horizontal boxes may contain several neighboring objects \cite{ding2018learning}.
...
In addition to architecture design, several works address the representation and learning of oriented bounding boxes. Angle prediction is challenging because rotated boxes can suffer from boundary discontinuities and ambiguous parameterizations. Classification-based approaches such as Circular Smooth Label (CSL) reformulate angle prediction as a classification problem to better handle angular periodicity. Kullback-Leibler-Divergence-based losses represent rotated bounding boxes as Gaussian distributions and compare predicted and ground-truth boxes at the distribution level. These methods improve the learning of oriented boxes and complement detector architectures such as RoI Transformer and Oriented R-CNN \cite{yang2022arbitrary,yang2021learning}.

Overall, the development from general object detectors to oriented object detectors reflects the specific requirements of aerial and satellite imagery. While two-stage and one-stage detectors provide the basic detection framework, remote-sensing applications require additional mechanisms for scale variation, dense scenes, and arbitrary object orientation. Therefore, models such as FPN, RetinaNet, RoI Transformer, and Oriented R-CNN are central references for understanding object detection in aerial imagery \cite{lin2017feature,lin2017focal,ding2018learning,xie2021oriented}.
